...
function _processTransitionEach(transition, routeAuth, cb) {
  var authRedirect = (routeAuth || '').redirect || __auth.options.authRedirect,
      ...
      excludes = toArray((routeAuth || '').excludes !== undefined ? routeAuth.excludes : undefined);
  ...
  if (routeAuth && (routeAuth === true || routeAuth.constructor === Array || isObject(routeAuth))) {
    ...
    } else if ((routeAuth.constructor === Array || isObject(routeAuth)) && (!compare(routeAuth, __auth.$vm.data[__auth.options.rolesKey]) || compare(excludes, __auth.$vm.data[__auth.options.rolesKey]))) {
